...
async def titan_scrape_stats(match_id: str, alias_helper: AliasHelper = None):
    """Scrape stats for a single match, using AliasHelper for team/league management."""
    ...
    if candidates:
        ...
        # ===== NEW: Add teams/leagues to alias.json using AliasHelper =====
        league = out.get("league", "")
        if out["home_team"]:
            alias_helper.add_team_if_new(out["home_team"], source="titan", league=league)
        if out["away_team"]:
            alias_helper.add_team_if_new(out["away_team"], source="titan", league=league)
        if league:
            alias_helper.add_league_if_new(league, source="titan")
        # Save changes
        alias_helper.save()
        # ===== END NEW =====
    ...
    # Get canonical names using AliasHelper
    out["home_team_canon"] = alias_helper.match_team(out.get("home_team", ""))
    out["away_team_canon"] = alias_helper.match_team(out.get("away_team", ""))
...
async def main():
    parser = argparse.ArgumentParser(description="Titan Stats Scraper with Skip Optimization (AliasHelper)")
    ...
    # Initialize AliasHelper
    alias_helper = AliasHelper()

    print("=" * 70)
    print("TITAN STATS SCRAPER - With AliasHelper")
    print("=" * 70)
    ...
    if args.id:
        ...
        out = await titan_scrape_stats(args.id, alias_helper)
    ...
    for idx, mid in enumerate(ids_to_scrape, start=1):
        ...
        out = await titan_scrape_stats(mid, alias_helper)
        ...
        if out.get("has_stats"):
            ...
            print(f"[{idx}/{total}] MISSING: {mid} | Sections: {len(out.get('sections_found', []))}")

    registry.save()
    write_report_excel(excel_path, full_rows, missing_rows)

    # Print alias stats
    alias_stats = alias_helper.get_stats()
    logger.info("=" * 60)
    ...
    logger.info(f"  New alias entries: {alias_stats['new_entries_this_session']}")
