...
Run:
  python hkjcscraper.py --id 50059564          # single event
  python hkjcscraper.py                        # bulk (home -> allodds)
  python hkjcscraper.py --max-events 20        # bulk limited
Options:
...
def alias_match(name: str, alias_map: dict) -> str:
    k = _norm_key(name)
...
class HKJCDetailedOddsScraper:
    ...
    def _parse_kickoff(time_raw: str) -> datetime | None:
        """
        HKJC format typically 'DD/MM/YYYY HH:MM'. Return naive datetime (local). If parsing fails, return None.
        """
        if not time_raw:
            return None
    ...
    async def scrape(self, event_id: str) -> dict:
        """
        Return dict with keys: status, event_id, data (when success).
        status ∈ {"success","skipped_existing","skipped_started","failed"}
        """
        # Skip rule 1: existing file with markets
        ...
        try:
            ...
            home_canon = alias_match(home, self.alias_map)
            away_canon = alias_match(away, self.alias_map)

            # Skip rule 2: kickoff in the past
...
class HKJCHomeScraper:
    ...
    async def ensure_rows_loaded(self, page, declared: int | None, max_clicks: int = 15, max_rounds: int = 25) -> int:
        """
        Keep clicking '顯示更多' until we reach the declared number of matches
        (if known) or until no new rows appear for a couple of rounds, or guards hit.
        """
        ...
        while True:
            ...
            if clicked:
                ...
                stagnant_rounds += 1  # no button found/visible this round
            ...
            if new_total > total:
                total = new_total
                stagnant_rounds = 0  # reset when we make progress
    ...
    async def extract_event_ids_from_dom(self, page) -> set[str]:
        """
        Extract event IDs directly from the DOM to avoid per-row navigation.
        Tries multiple attribute sources and regexes.
        """
        js = """
        (() => {
    ...
    async def scrape(self):
        """
        Returns (results, declared_count, extracted_count)
        - results: list of rows with event_id
        """
        results: list[dict] = []
        ...
        async with async_playwright() as p:
            ...
            # Extract all event IDs in one pass (fast path)
            ids = await self.extract_event_ids_from_dom(page)
            logger.info("Extracted %d event ids from DOM", len(ids))

            # If we still have fewer than declared, try another expansion pass
...
async def main():
    ...
    cprint("🚀 HKJC standalone odds scraper")
